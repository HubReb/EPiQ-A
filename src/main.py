...
parser.add_argument("--mode", action = 'store', default = 'evaluate',
                    type = str, choices = ["evaluate", "interactive"],
                    help="Determines the program you want to run",
                    )
parser.add_argument("--article-retrieval-model", action = 'store', default = 'tfidf',
                    type = str, choices = ["bm", "tfidf"],
                    help="Determines the model you want to use for article retrieval",
                    )
parser.add_argument("--article-retrieval-model-filename", action = 'store',
                    default = TFIDF_MODEL, type = str,
                    choices = [BM25_MODEL, TFIDF_MODEL],
                    help="Determines file you want to load your articel "
                    "retrieval model from",
                    )
parser.add_argument("--no-index", action = 'store_false',
                    help="Switches inverted index off",
                    )
parser.add_argument("--no-retrieval", action = 'store_false',
                    help="Switches article retrieval off, retrieval based "
                    "only on paragraphs",
                    )
parser.add_argument("--num-articles", action = 'store', default = 10, type = int,
                    help="Number of articles to retrieve",
                    )
parser.add_argument("--paragraph-level", action = "store", default = "paragraph",
                    type = str, choices = ["paragraph", "sentence"],
                    help = "Determines the paragraph splitting level"
                    )
parser.add_argument("--num-paragraphs", action = 'store', default = 10, type = int,
                    help="Number of paragraphs to retrieve",
                    )
parser.add_argument("--max-context-size", action = 'store', default = 400, type = int,
                    help="Maximum tokens per context (for answer extraction)",
                    )
parser.add_argument("--transformers-model-name", action = 'store', default = None,
